	...
	$.getJSON(
		...
		function(data) {
			...
			for (var i in data.borrowList) {
				...
				bookApplyConD.dcBl = data.borrowList[i].delCode;
				
				switch (bookApplyConD.dcBl) {
				
				case '3' :
					bookApplyConD.dcBl = $('<td>').append($('<a>', {
						text:'독서 완료',
						href:'#',
				...
				case '6' :
					bookApplyConD.dcBl = $('<td>').text('나눔 가능');
					break;
				}
...
//내가 빌린 책(기부) 반납 현황에서 반납을 클릭했을 때 발생하는 이벤트를 위한 함수
